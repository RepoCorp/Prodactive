...
namespace MongoProviders
{
    ...
    public sealed class MongoSessionStateStore : SessionStateStoreProviderBase
    {
        ...
        private MongoCollection<BsonDocument> GetSessionCollection(MongoServer conn)
        {
            ...
            return conn.GetDatabase(_db).GetCollection(_collection);
        }
        ...
        public override void Initialize(string name, System.Collections.Specialized.NameValueCollection config)
        {
            ...
            _db = _connectionString.Substring(_connectionString.IndexOf("/", 11)+1);

            if (config["collectionSuffix"] != null)
            {
                _collection = Helper.GenerateCollectionName(_applicationName, config["collectionSuffix"]);
            }
        ...
        private SessionStateStoreData GetSessionStoreItem(bool lockRecord, HttpContext context, string id,out bool locked, out TimeSpan lockAge, out object lockId, out SessionStateActions actionFlags)
        {
            ...
            try
            {
                ...
                if (foundRecord && !locked)
                {
                    ...
                    item = actionFlags == SessionStateActions.InitializeItem ? CreateNewStoreData(context, (int)_config.Timeout.TotalMinutes) : Deserialize(context, serializedItems, timeout);
                }
            }
            catch (Exception e)
            {
